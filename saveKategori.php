...
for ($i = 0; $i < count($data_id_variabel); $i++) {
    ...
    for ($j = 0; $j < count($data_id_kategori); $j++) {
        $sql = "UPDATE categories SET namaKategori='".$data_nama_kategori[$j]."',batasAtas=".$data_batas_atas[$j].",batasTengah=".$data_batas_tengah[$j].",batasBawah=".$data_batas_bawah[$j].",deleted=0 WHERE id = $data_id_kategori[$j]";
        echo $sql."<br/>";
        mysqli_query($link,$sql);    
    }
